                ...
                <div class="day-container">
                    <div v-for="(item,index) in dateContainer" class="day-box" :key="index" :style="{'color':item.currentMonth?'#777':'#bbb'}">
                        ...
                        <div class="activity-point">
                            ...
                            width="320"
                            v-model="visible">
                            <p style="margin:3px 0px;padding:0px;font-size:12px;" v-for="(it,idx) in item.activitys" :key="idx">
                                <el-link type="default" :underline="false" style="font-size:12px;"><span style="font-size:20px;font-weight:bold;margin-right:8px;position:relative;top:2px;">·</span>{{it.title}}</el-link>
                            </p>
                            
                            <div slot="reference" style="padding:0px;">
                                <span v-show="item.activitys.length>0"  style="display:block;width:6px;height:6px;background:#fff;border:1px solid #108eff;margin:auto;border-radius:50%;float:center;"></span>
                            </div>
                            </el-popover>
            ...
            <div class="silder-box" v-for="(item,index) in silderList" :key="index">
                ...
                <div class="activity-info">
                    ...
                    <p class="activity-address"> 
                        ...
                        <el-button size="mini" :type="item.outtime?'info' :'primary'" style="float:right;position:relative;top:-5px;" round>&nbsp;&nbsp;{{item.outtime?'活动详情':'报名参加'}}&nbsp;&nbsp;</el-button>
                    </p>
                </div>
            </div>
        </div>
